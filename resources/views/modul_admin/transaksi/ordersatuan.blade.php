...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">
                Transaksi Satuan
                <a href="{{ url('add-order-satuan') }}" class="btn btn-primary">Tambah</a>
            </h4>
                                ...
                                <td>
                                    @if ($item->status_payment == 'Pending')
                                        <a class="btn btn-sm btn-danger" data-toggle="modal"
                                            ...
                                            data-id-bayar="{{ $item->status_payment }}" id="klicksatuan"
                                            data-target="#ubah_status_pay_satuan" style="color:white">Bayar</a>
                                ...
                                <td>
                                    @if ($item->status_order == 'Antrian')
                                        <a class="btn btn-sm btn-warning" data-toggle="modal" data-id="{{ $item->id }}"
                                            ...
                                            id="klikmodalsatuan" data-target="#ubah_status_satuan"
                                            style="color:white">Antrian</a>
                                    @elseif ($item->status_order == 'Process')
                                        <a class="btn btn-sm btn-primary" data-toggle="modal" data-id="{{ $item->id }}"
                                            data-id-nama="{{ $item->customer }}" data-id-order="{{ $item->status_order }}"
                                            id="klikmodalsatuan" data-target="#ubah_status_satuan"
                                            style="color:white">Proses</a>
            ...
            @include('modul_admin.transaksi.statusordersatuan')
            @include('modul_admin.transaksi.statusbayarsatuan')
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).on('click', '#klikmodalsatuan', function() {
        ...
        $(document).on('click', '#save_status_satuan', function() {
            var id = $("#id").val();
            var customer = $("#customer").val();
            var status_order = $("#status_order").val();

            $.ajax({
                url: '{{ Url('ubah-status-order-satuan') }}',
                method: 'GET',
                data: {
        ...
        $(document).on('click', '#klicksatuan', function() {
            ...
            $('#ubah_status_pay_satuan').modal('show');
        });

        // Proses Ubah Status Pembayaran
        $(document).on('click', '#simpan_status_satuan', function() {
            var id = $("#id_bayar").val();
        ...
        $('#formKetDelivery').submit(function(e) {
            ...
            $.ajax({
                url: '{{ url('update-ket-delivery-satuan') }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: id,
